...
#define GO_ADDR UINT64_C(0x0)
#define N_ADDR UINT64_C(0x4)
#define DONE_ADDR UINT64_C(0x8)
#define RESULT_ADDR UINT64_C(0xC)
...
//software implementation of fibonacci calculator
...
int peek_poke_example(int slot_id, int pf_id, int bar_id) {
	...
	for (i=1; i<=30; i++){		
		rc = fpga_pci_poke(pci_bar_handle, N_ADDR, i);
		fail_on(rc, out, "Unable to write to the fpga !");
		
		go = 0x1;
		rc = fpga_pci_poke(pci_bar_handle, GO_ADDR, go);
		fail_on(rc, out, "Unable to write to the fpga !");
		...
		rc = fpga_pci_poke(pci_bar_handle, DONE_ADDR, 0);
		fail_on(rc, out, "Unable to write to the fpga !");
		
		while(done == 0){
			rc = fpga_pci_peek(pci_bar_handle, DONE_ADDR, &done);
			fail_on(rc, out, "Unable to read read from the fpga !");
		}
		
		rc = fpga_pci_poke(pci_bar_handle, RESULT_ADDR, 0);
		fail_on(rc, out, "Unable to write to the fpga !");
	 
		rc = fpga_pci_peek(pci_bar_handle, RESULT_ADDR, &value);
		fail_on(rc, out, "Unable to read read from the fpga !");
